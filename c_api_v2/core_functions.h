			...
			case '|':
				...
				if ( str1[0]!='\0' && str2[0]!='\0')
				{
					...
					else
					{
						...
						char attr[9][1024] ;//to increase size of fs values to handle the unicode string in feature structure
...
char * make_or_node_to_string ( or_node *OR)
{
	...
	char *str=(char *)malloc(2000*sizeof(char));//change the size of an or node to handle the unicode string in or node
	str[0]='\0';
...
char *make_fs_struct_to_string ( fs_struct *fs)
{
	...
	{
		int i;
		char *str=(char *)malloc(1000*sizeof(char));//change the size of feature structure to handle the unicode string in feature structure
		strcpy(str,"<fs ");
		GList *start=fs->keys;
		char *tmp;
